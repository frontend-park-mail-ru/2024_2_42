 ...
 * @param {boolean} [rules.isUserName=false] - If true, validates the input as a nickname (3-20 characters, only digits, letters, and underscores allowed).
...
export const validateInput = (input, rules) => {
    ...
    if (rules.isPassword) {
        if (input.length < 8 || input.length > 24) {
            return {
                isValid: false,
                error: 'Пароль должен содержать от 8 до 24 символов.',
            };
        }
        if (!/[A-Z]/.test(input)) {
            return {
...
export const getCaptionSetForRule = (rules) => {
    if (rules.isUserName) {
        return ['Имя должно содержать:', '• От 3 до 20 символов.', "• Только буквы, цифры и символы '_'."];
    }
    ...
    if (rules.isPassword) {
        return ['Пароль должен содержать:', '• От 8 до 24 символов.', '• Хотя бы одну заглавную букву.', '• Хотя бы одну цифру.'];
    }
