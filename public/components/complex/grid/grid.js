...
export class GridComponent extends BaseComponent {
  ...
  constructor(parent, pins, marginTop) {
    ...
    window.addEventListener(
      ...
      (event) => {
        ...
        const pageDarkBlurBack = document.getElementById(
          'page__dark-blur-container'
        );
        if (pageDarkBlurBack) {
          pageDarkBlurBack.style.height = height + 'px';
        }
        const pageTransparentBlurBack = document.getElementById(
          'page__transparent-container'
        );
  ...
  renderTemplate() {
    ...
    if (this.#pins && this.#pins.length > 0) {
      for (const pinData of this.#pins) {
        pinData.media_url = pinData.media_url.replace(
          'http://minio:9000',
          'http://localhost:9000'
        );
      ...
      pins: pinsToRender
        .map((pin) => {
          if (typeof pin.renderTemplate === 'function') {
            console.log('pin building', pin.renderTemplate());
  ...
  buildLayout() {
    ...
    const columnWidth =
      (this.parentContainerWidth - widthGutter * (columnsN + 1)) / columnsN;
    ...
    for (const pin of this.#pins) {
      const pinContainer = document.querySelector(
        `.pin__content-container-${pin.pin_id}`
      );
      if (pinContainer) {
        ...
        pinContainer.style.top = `${
          heights[minIdx] + heightGutter + headerHeight
        }px`;
        pinContainer.style.left = `${
          minIdx * columnWidth + widthGutter * (minIdx + 1)
        }px`;
  ...
  get adaptiveColumnsN() {
    let newColumnN = Math.floor(
      this.parentContainerWidth / this.parentContainerColumnsRatio
    );
  ...
  buildPinPreview(pin) {
    ...
    const pinPreviewBtn = document.querySelector(
      `.pin__image-preview-button-${pin.pin_id}`
    );
    ...
    pinPreviewBtn.addEventListener('click', (event) => {
      ...
      const pinImageElement = document.querySelector(
        `.pin__image-${pin.pin_id}`
      );
      if (pinImageElement) {
        ...
        const previewTopSide = document.querySelector(
          '.preview__stats-bookmarks-container'
        );
        const previewBottomSide = document.querySelector(
          '.preview__author-container'
        );
        const previewRightSide = document.querySelector(
          '.preview__side-menu-container'
        );
        ...
        if (
          ...
          previewContainer.style.width = `${
            intWidth * PREVIEW_IMG_X_FACTOR + previewRightSide.clientWidth
          }px`;
          previewContainer.style.height = `${
            intHeight * PREVIEW_IMG_X_FACTOR +
            previewTopSide.clientHeight +
            previewBottomSide.clientHeight
          }px`;
  ...
  createBackgroundListeners() {
    ...
    if (!document.getElementById('page__dark-blur-container')) {
      ...
      pageBluredBackground.addEventListener('click', (event) => {
        ...
        const pinPreview = document.querySelector(
          '.preview__content-container'
        );
  ...
  bookmarkListenerHandler(bookmarkElement, previewState) {
    ...
    document.querySelector('.preview__stats-bookmarks-number').textContent =
      previewState.BookmarksNumber;
  }
  ...
  saveBtnListenerHandler(boards) {
    if (document.querySelector('.details-menu__content-container')) {
      const detailsMenuContainer = document.querySelector(
        '.details-menu__content-container'
      );
      document
        .querySelector('.preview__side-menu-more-container')
        .removeChild(detailsMenuContainer);
      this.#moreBtnClicked = false;

      const pageTransparentBackground = document.getElementById(
        'page__transparent-container'
      );
    ...
    if (this.#saveBtnClicked) {
      document.querySelector('.preview__board-selector').style.background =
        'no-repeat right 0.8em center / 1.4em';
      const saveBtnIcon = document.querySelector(
        '.preview__board-selector-container-icon'
      );
      saveBtnIcon.style.transform = 'none';
      saveBtnIcon.src = './assets/icons/expand-list.svg';
      document.querySelector(
        '.preview__board-selector-container-text'
      ).style.color = 'white';
      this.#saveBtnClicked = false;

      const previewBoardSelectorContainer = document.querySelector(
        '.preview__board-selector'
      );
      const saveBoxContainer = document.querySelector(
        '.savebox__content-container'
      );
      previewBoardSelectorContainer.removeChild(saveBoxContainer);

      const transparentContainer = document.getElementById(
        'page__transparent-container'
      );
      ...
      previewBoardSelectorContainer.insertAdjacentHTML(
        'beforeend',
        renderedSaveBox
      );

      const saveBoxContentContainer = document.querySelector(
        '.savebox__content-container'
      );
      ...
      saveBoxContentContainer.style.top =
        saveBtnContainer.clientHeight + saveBoxMarginTop + 'px';
  ...
  addSaveBtnListener(boards) {
    ...
    document
      .querySelector('.preview__board-selector-container')
  ...
  moreBtnListenerHandler(options) {
    if (this.#moreBtnClicked) {
      ...
      document
        .querySelector('.preview__side-menu-more-container')
        .insertAdjacentHTML('beforeend', renderedDetailsMenu);

      this.addTransparentLayoutForDetailsMenu();

      const detailMenuContainer = document.querySelector(
        '.details-menu__content-container'
      );
      const menuMoreElement = document.querySelector(
        '.preview__side-menu-more'
      );

      const detailMenuContainerMarginTop = -15;
      detailMenuContainer.style.top =
        menuMoreElement.clientHeight + detailMenuContainerMarginTop + 'px';

      detailMenuContainer.style.left =
        -((detailMenuContainer.clientWidth - menuMoreElement.clientWidth) / 2) +
        'px';
  ...
  addMoreBtnListener(options) {
    ...
    document
      .querySelector('.preview__side-menu-more')
      .addEventListener('click', (event) => {
        event.preventDefault();
  ...
  addInputClearBtnListener(boards) {
    const searchInputClearIcon =
      document.querySelector(`.savebox__content-main-box
														.searchinput__content-container
  ...
  searchInputListenerHandler(inputField, boards) {
    ...
    const boardsListContainer = document.querySelector(
      '.savebox__boards-list-container'
    );
  ...
  addSaveToBoardListener(boards) {
    for (const board of boards) {
      document
        .getElementById(`checkbox-${board.BoardName}`)
