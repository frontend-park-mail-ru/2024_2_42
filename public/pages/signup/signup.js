...
export class SignUpComponent {
    ...
    constructor(parent, inputsData, buttonData, buttonFooterData) {
        ...
        this.#inputsData.userName.captions = getCaptionSetForRule({ isUserName: true });
        this.#inputsData.email.captions = getCaptionSetForRule({ isEmail: true });
        this.#inputsData.password.captions = getCaptionSetForRule({ isPassword: true });
    }
    ...
    addInputFocusHandler() {
        const [userNameInput, loginInput, passwordInput] = document.getElementsByClassName('input');
        const [userNameCaptionsBlock, emailCaptionsBlock, passwordCaptionsBlock] = document.getElementsByClassName('input__helper-text-list');
    ...
    addInputOnChangeHandler() {
        ...
        const passwordRepeatErrTextBlock = document.createElement('input__error-text');

        userNameInput.addEventListener('input', (event) => {
            event.preventDefault();

            const inputState = validateInput(userNameInput.value, { isUserName: true });
        ...
        passwordInput.addEventListener('input', (event) => {
            ...
            if (passwordRepeatInput.value.length > 0) {
                const inputState = validateInput(passwordInput.value, { compareWith: passwordRepeatInput.value });
            ...
            const inputState = validateInput(passwordInput.value, { isPassword: true });
        ...
        passwordRepeatInput.addEventListener('input', (event) => {
            ...
            const inputState = validateInput(passwordRepeatInput.value, { compareWith: passwordInput.value });
